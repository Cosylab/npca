...
/* Added not to use external .h files */
...
#define HAS_WIRE ((EPICS_REVISION >= 14) && (EPICS_MODIFICATION >= 9))
...
static volatile bool repeater_stopped = false;

// declaration of method declared in udpiiu.cpp
 ...
 *  ca_repeater_stop ()
 */
epicsShareFunc void ca_repeater_stop ()
{
    // set stop flag
    repeater_stopped = true;

    ///
    /// ... and sent a message to repeater to wake up its thread to exit
    ///
    SOCKET sock = epicsSocketCreate ( AF_INET, SOCK_DGRAM, 0 /*IPROTO_UDP*/ );
    if ( sock == INVALID_SOCKET ) {
        return;
    }

    unsigned short port = envGetInetPortConfigParam ( & EPICS_CA_REPEATER_PORT,
                                       static_cast <unsigned short> (CA_REPEATER_PORT) );
    caRepeaterRegistrationMessage(sock, port, 0);
 ...
 *  ca_repeater ()
 */
epicsShareFunc void ca_repeater () 
{
    tsFreeList < repeaterClient, 0x20 > freeList;
    int size;
    SOCKET sock;
